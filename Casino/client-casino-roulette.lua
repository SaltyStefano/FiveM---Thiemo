...
AddEventHandler('route68_ruletka:start', function()
	ESX.TriggerServerCallback('mrpx_roulette:check_money', function(quantity)
		if quantity >= 10 then
			SendNUIMessage({
				type = "show_table",
				zetony = quantity
...
RegisterNUICallback('roll', function(data, cb)
	cb('ok')
	TriggerEvent('mrpx_roulette:start_game', data.kolor, data.kwota)
end)

RegisterNetEvent('mrpx_roulette:start_game')
AddEventHandler('mrpx_roulette:start_game', function(action, amount)
	local amount = amount
	if game_during == false then
		TriggerServerEvent('mrpx_roulette:removemoney', amount)
		...
		if action == 'black' then
			if has_value(black, randomNumber) then
				...
				TriggerServerEvent('mrpx_roulette:givemoney', action, amount)
			else
